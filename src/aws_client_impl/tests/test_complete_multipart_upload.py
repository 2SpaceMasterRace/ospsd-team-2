...
def test_complete_multipart_upload_returns_true_on_success(
    ...
    assert ok is True
    fake_boto_client.complete_multipart_upload.assert_called_once_with(
        Bucket="my-bucket",
        Key="k",
        UploadId="u",
...
def test_complete_multipart_upload_returns_false_on_client_error(
    ...
    fake_boto_client = mocker.Mock()
    fake_boto_client.complete_multipart_upload.side_effect = _client_error()
    c = _make_client(mocker, fake_boto_client)
    ...
    assert ok is False
    fake_boto_client.complete_multipart_upload.assert_called_once()
