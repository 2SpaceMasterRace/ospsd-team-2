...
def _client_error(op="DownloadFileobj"):
    return ClientError(
...
def test_download_fileobj_returns_true_on_success(mocker, tmp_path):
    fake_s3 = mocker.Mock()
    ...
    args, kwargs = fake_s3.download_fileobj.call_args
    assert args[0] == "my-bucket"
    assert args[1] == "my-key"
    assert hasattr(args[2], "write")  # file handle


def test_download_fileobj_returns_false_on_client_error(mocker, tmp_path):
    fake_s3 = mocker.Mock()
