...
def test_create_bucket_us_east_1_does_not_send_location_constraint(mocker):
...
def test_create_bucket_other_region_sends_location_constraint(mocker):
    fake_client = mocker.Mock()
    mocker.patch("aws_client_impl.s3_client.boto3.client", return_value=fake_client)
...
def test_create_bucket_returns_false_on_client_error(mocker):
    fake_client = mocker.Mock()
